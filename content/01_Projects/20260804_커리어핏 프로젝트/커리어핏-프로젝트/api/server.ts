...
function parseJDFallback(inputText: string, url: string | undefined) {
  // J&J APM Electrophysiology 전용 Fallback 예외 처리
  if (inputText.includes("Associate Product Manager - Electrophysiology") && (inputText.includes("Support annual marketing plans") || (url && (url.includes('4445699622') || url.includes('linkedin.com/jobs/view'))))) {
    return {
      companyName: "Johnson & Johnson MedTech",
      title: "[Johnson & Johnson MedTech] Associate Product Manager - Electrophysiology",
      position: "Associate Product Manager",
      dueDate: "데드라인 미정 (상시/채용시 마감)",
      tasks: [
      ...
      keywords: ["Product Management", "Medical Devices", "Electrophysiology", "Brand Marketing", "Data Analysis", "Project Management", "Korean", "English"],
  ...
  // 간단한 키워드 추출 (해시태그나 주요 기술 스택 감지)
  const knownKeywords = [
    "React", "TypeScript", "Node.js", "Next.js", "Python", "Java", "Go", "Kubernetes", 
    "Docker", "AWS", "Flutter", "Kotlin", "Swift", "Figma", "GA4", "SQL", "Git", "C++", 
    "Vue.js", "Vue", "Spring", "Django", "FastAPI"
  ];
  ...
  for (let line of lines) {
    ...
    if (/주요\s*업무|담당\s*업무|할\s*일|역할|role|responsibilities|tasks/i.test(lowerTrimmed)) {
      currentSection = 'tasks';
      continue;
    } else if (/필수\s*자격|필수\s*요건|자격\s*요건|지원\s*자격|requirements|eligibility|requirements/i.test(lowerTrimmed)) {
      currentSection = 'requirements';
      continue;
    } else if (/우대\s*사항|우대\s*조건|선호\s*요건|preferred|qualifications/i.test(lowerTrimmed)) {
      currentSection = 'preferred';
      continue;
    } else if (/혜택|복지|전형\s*절차|근무\s*조건|benefits|process|전형\s*과정/i.test(lowerTrimmed)) {
      currentSection = 'etc';
      continue;
    }

    // 목록 형태(- 또는 * 또는 숫자)로 시작하면 데이터 추가
    if (trimmed.startsWith('-') || trimmed.startsWith('*') || trimmed.startsWith('•') || /^\d+[\s.)]/.test(trimmed)) {
      const content = trimmed.replace(/^[-*•\s]+|^\d+[\s.)]+\s*/, '');
      if (content.trim()) {
        if (currentSection === 'tasks') {
          tasks.push(content.trim());
        } else if (currentSection === 'requirements') {
          requirements.push(content.trim());
        } else if (currentSection === 'preferred') {
          preferred.push(content.trim());
        }
      }
    } else {
      // 목록 기호가 없더라도 의미 있는 텍스트 줄이면 단락에 추가
  ...
  // 만약 특정 섹션이 아예 비어있다면 원문 텍스트 전체에서 해당 키워드가 들어간 부분을 긁어오거나, 최소한 문장 전체를 보존하기 위해 다음과 같이 채운다.
  if (tasks.length === 0) {
    tasks.push("공고 원문 텍스트를 참조해주세요.");
  }
  if (requirements.length === 0) {
    requirements.push("공고 원문 텍스트를 참조해주세요.");
  }
  if (preferred.length === 0) {
    preferred.push("공고 원문 텍스트를 참조해주세요.");
  }
  ...
  // 첫 몇 줄에서 대괄호나 특정 문구로 추출 시도
  const firstFewLines = lines.slice(0, 5).map(l => l.trim()).filter(l => l.length > 0);
  if (firstFewLines.length > 0) {
    const matchCompany = firstFewLines[0].match(/\[(.*?)\]/);
    if (matchCompany && matchCompany[1]) {
      companyName = matchCompany[1];
      title = firstFewLines[0];
    } else {
      title = firstFewLines[0].slice(0, 50);
      if (title.includes(' - ')) {
  ...
  // 마감일 추출: 상시 채용이나 채용시 마감을 뜻하는 영문/국문 키워드가 있는지 검사
  let dueDate = "데드라인 미정 (상시/채용시 마감)";
  const isAlwaysOpen = /until\s*filled|until\s*the\s*position\s*is\s*filled|상시\s*채용|채용시\s*마감|TBD|미정|상시|always/i.test(inputText);

  // 날짜 형식(YYYY-MM-DD, YYYY.MM.DD, YYYY/MM/DD 등) 정규식
...
[중요 언어 및 보존 지침]
- **원문 언어 보존**: 채용 공고 본문이 영어로 작성되어 있다면 주요 업무(tasks), 필수 자격요건(requirements), 우대 사항(preferred) 등의 텍스트를 한국어로 번역하지 마시고 **영어 원문 그대로** 추출하세요. 한국어로 작성되어 있다면 한국어 원문 그대로 추출하세요.
- **키워드 형식**: 핵심 기술/역량 키워드(keywords) 역시 공고 본문의 원래 언어(예: 영어 공고라면 "Product Management" 등 영어 키워드)를 존중하여 추출하세요.
      `;
...
${url ? `\n\n원본 공고 URL: ${url}` : ''}`;
        ...
        return res.json({
          ...
          data: {
            ...parsedJson,
            rawText: inputText, // AI가 축약한 텍스트가 아닌, 오리지널 입력 텍스트 전체를 무조건 보존
            originalUrl: url || '',
    ...
    const fallbackData = parseJDFallback(inputText, url);
    return res.json({
      success: true,
      data: {
        ...fallbackData,
