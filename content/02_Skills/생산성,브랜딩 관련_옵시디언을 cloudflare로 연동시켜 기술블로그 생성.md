---
title: 생산성,브랜딩 관련_옵시디언을 cloudflare로 연동시켜 기술블로그 생성
tags:
  - 기술브랜딩
  - 브랜딩
  - 생산성
publish: true
---
생각지도 않은 변수들 때문에 생각보다 과정이 쉽진 않았습니다!
그렇지만 꼭 ! ai가 잘 읽는다는 옵시디언으로 
기술블로그를 운영하고 포트폴리오를 만들고자 하는 집념으로 
금요일부터 약 5일간 매달려서 드디어 성공했는데요!
...
!!이 상태에서 주의하실 사항: 
 ...
 - git을 까신 상태셔야 합니다.
![[옵시디언을 cloudflare로 연동시키기2.png]]


![[옵시디언을 cloudflare로 연동시키기3.png]]
...
오류가 계속 나서(cloudflare와 로컬 사이의 무언가 문제인 듯 함)

build command는 제미나이의 도움을 받아
(5일간 계속 수정하고 무수히 많이 프로젝트를 민 결과)
  
node quartz/bootstrap-cli.mjs build --verbose 로 했습니다.

(cloudflare에서 배포하는 과정에서 quartz5 관련 못 읽는다고 오류가 많이 났음)

![[옵시디언을 cloudflare로 연동시키기4.png]]
...
또한 아주 중요한 부분으로!!! NODE 버젼 관련해서 quartz가 다루는 버젼이랑
계속 배포과정에서 문제가 많았어서 
아예 변수로 node_version 24.18.0으로 추가시켰습니다.
(입력할때는 type에 text라고 뜸)

22이상이면 되는데 실제 로컬 버젼이랑 일치시키는 22이상이면 됩니다.
(저는 24.18.0 버젼이었어서 이렇게 맞춤)
![[옵시디언을 cloudflare로 연동시키기5.png]]
